...
bool AddItem(const Item * pi, Tree * ptree)
{
	...
	if ((seek_node = SeekItem(pi, ptree).child) && seek_node != NULL)
	{
		++(seek_node->item.repeat);
		return true;
	}
...
static void AddNode(Trnode * new_node, Trnode * root)
{
	if (ToLeft(&new_node->item, &root->item))
	{
		if (root->left == NULL) // пустое поддерево
			root->left = new_node; // поэтому добавить узел сюда
		else
			AddNode(new_node, root->left); // иначе обработать дерево
	}
...
// адрес родительского элемента, указывающего на целевой узел
{
	...
	{
		// выяснение места присоединения поддерева
